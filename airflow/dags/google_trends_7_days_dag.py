...
class GooogleTrendsMonthly:
    ...
    def create_dag(self)-> DAG:
        ...
        dag: DAG = DAG(
            ...
            description='update google trends 7 days',
            max_active_runs = 1,
            catchup=False)

        update_monthly: PythonOperator = PythonOperator(
            task_id='update_monthly',
            python_callable=self.GoogleAnalyzes.past_7_days,
            op_args=[],
            provide_context=True,
            dag=dag
        )
        tasks_7_days.append(update_monthly)

        for i in range(len(tasks_7_days) - 1):
            tasks_7_days[i] >> tasks_7_days[i + 1]
        return dag
